...
from release_tracker.enrich import reset_theme_gate
...
def reset_process_caches() -> Iterator[None]:
    ...
    Both exist to keep an interactive add fast — a shared IGDB app token, and a latch that stops
    re-calling an LLM whose key/billing is rejected — and both would otherwise let one test's
